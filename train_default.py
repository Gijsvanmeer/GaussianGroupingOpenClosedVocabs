...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb, save_param, save_file,):
    first_iter = 0
    ...
    if save_param:
        ...
        loss_vals = []
        obj_loss_vals = []
        save_file = save_file + "original_method"
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if save_param:
            obj_loss_vals.append(loss_obj.item())
    ...
    if save_param:
        ...
        version = 1
        while os.path.isfile("GaussianGroupingBase_" + str(version) + ".png"):
            version += 1
        filename = "GaussianGroupingBase_" + str(version) + ".png"
        plt.plot(np.arange(0, len(obj_loss_vals), 1, dtype=int), obj_loss_vals)
        plt.title("Loss progress of object loss default method.")
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.use_wandb, args.save_param, args.save_file,)
