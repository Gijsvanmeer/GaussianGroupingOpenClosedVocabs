...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb, use_dl3, lmd, dl3_classes, save_param, save_file, dl3_path):
    first_iter = 0
    prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians, use_dl3=True, dl3_path=dl3_path)
    ...
    if save_param:
        ...
        save_file = save_file + "secondary_loss_lmd_" + str(lmd) + ".txt"
    else:
        save_file_eval = None
    ...
    for iteration in range(first_iter, opt.iterations + 1):  
        ...
        if save_param:
            secondary_losses.append(loss_obj_dl3.item())
        ...
        if iteration % opt.reg3d_interval == 0:
            ...
            # Do the same for the closed vocab model
    ...
    if save_param:
        ...
        # Loss progression of new method
        h = open(save_file, "a")
        ...
        filename = "secondary_loss_double_" + str(lmd) + "_" + data_name + ".png"
        if os.path.isfile(filename):
            version = 1
            while os.path.isfile("secondary_loss_double_" + str(lmd) + "_" + data_name + "_" + str(version) + ".png"):
                version += 1
            filename = "secondary_loss_double_" + str(lmd) + "_" + data_name + "_" + str(version) + ".png"
        plt.plot(np.arange(0, len(secondary_losses), 1, dtype=int), secondary_losses)
        plt.title("Loss progress of lamda " + str(lmd) + ".")
...
if __name__ == "__main__":
    ...
    available_methods = [1, 2, 3, 4, 5, 6, 7, 8, 9]
    parser.add_argument('--ip', type=str, default="127.0.0.1")
    parser.add_argument('--closed_vocab_path', type=str, default=None)
    parser.add_argument('--use_dl3', type=bool, default=True)
    ...
    parser.add_argument('--method', type=int, default=1, choices=available_methods)
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.use_wandb, args.use_dl3, args.lmd, args.dl3_classes, args.save_param, args.save_file, args.closed_vocab_path)
