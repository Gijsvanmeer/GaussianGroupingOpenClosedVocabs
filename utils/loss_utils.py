...
def MIoU(gt_image, pred_image, num_classes=256):
    """
    Calculate the Mean Intersection over Union (MIoU) between a ground truth and predicted image.


    :param gt_image: Tensor of the ground truth image.
    :param pred_image: Tensor of the predicted image.

    :return: Computed MIoU value (single float value)
    """
    if len(gt_image.shape) != 2 or len(pred_image.shape) != 2:
        assert False, "Incorrect dimensionality for images, gt_image has " + str(len(gt_image.shape)) + " dimensions, and the predicted image has " + str(len(pred_image.shape)) + " dimensions, both should be of dimensionality: 2"
    # Flatten images
    gt_flat = torch.flatten(gt_image).cpu().detach().numpy()
    pred_flat = torch.flatten(pred_image).cpu().detach().numpy()
    
    # Obtain class frequencies
    gt_freq = np.bincount(gt_flat)
    pred_freq = np.bincount(pred_flat)
    
    # # Setup confusion matrix
    conf_mtx = confusion_matrix(gt_flat, pred_flat, labels=np.arange(num_classes))

    
    # Calculate intersection and union
    I = np.diagonal(conf_mtx)
    if len(gt_freq) != num_classes:
        p1d = (0, len(I) - len(gt_freq))
        gt_freq = np.pad(gt_freq, p1d, 'constant')

    if len(pred_freq) != num_classes:
        p1d = (0, len(I) - len(pred_freq))
        pred_freq = np.pad(pred_freq, p1d, 'constant')
    freqs = gt_freq + pred_freq
